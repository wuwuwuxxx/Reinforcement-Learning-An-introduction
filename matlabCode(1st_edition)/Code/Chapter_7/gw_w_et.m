function [Q,ets] = gw_w_et(alpha,lambda,sideII,sideJJ,s_start,s_end,wind,MAX_N_EPISODES)
% GW_W_ET - Performs on-policy sarsa iterative action value funtion estimation 
% for the windy grid world example using elabability traces
% 
% Written by:
% -- 
...
Q = zeros(nStates,nActions); ET = zeros(nStates,nActions); 
...
for ei=1:MAX_N_EPISODES,
  ...
  while( 1 ) 
    ...
    if( ~( (stp1(1)==s_end(1)) && (stp1(2)==s_end(2)) ) ) % stp1 is not the terminal state
      delta = rew + gamma*Q(stp1i,atp1) - Q(sti,at);
    ...
    if( ACC_ET )
      ET(sti,at) = ET(sti,at)+1;     % <- accumulating elagabiity traces ... 
    else
      ET(sti,at) = 1;                % <- replacing elagabiity traces ... 
    end
    % update Q and ET 
    Q  = Q + alpha*delta*ET;
    ET = gamma*lambda*ET; 
...
%if( (stp1(1)==s_end(1)) && (stp1(2)==s_end(2)) )
  %rew=+1;
  rew=0;
else
  rew=-1;
