% SHORTCUT_MAZE_SCRIPT - Implements the DynaQ Algorithm on the simple maze example found in Chapter 9
% 
% Written by:
% -- 
...
MZ = mk_ex_9_3_mz(0); [sideII,sideJJ] = size(MZ); 
...
for npsi=1:length(nPSV),
  ...
  [Q,ets,cr] = dynaQplus_maze(alpha,epsilon,gamma,kappa,nPlanningSteps,@mk_ex_9_3_mz,s_start,s_end,MAX_N_STEPS);
  allCR(npsi,:) = cr(2:end); 
  fhl{npsi} = sprintf('dynaQplus: %d planning steps',nPlanningSteps); 
  
  [Q,ets,dum1,dum2,dum3,cr] = dynaQ_maze(alpha,epsilon,gamma,nPlanningSteps,@mk_ex_9_3_mz,s_start,s_end,MAX_N_STEPS);
  allCR(length(nPSV)+npsi,:) = cr(2:end); 
  fhl{length(nPSV)+npsi} = sprintf('dynaQ: %d planning steps',nPlanningSteps); 
end

figure; fhs = plot( (1:6000), allCR(:,2:(6000+1)), '-' ); 
title( 'cummulative reward' ); grid on;
xlabel('timestep index'); ylabel('cum. reward'); drawnow; 
legend( fhs, fhl, 'Location', 'NorthWest' );
fn = sprintf('shortcut_dynaQplus_vs_dynaQ_cum_reward'); saveas( gcf, fn, 'png' ); 
