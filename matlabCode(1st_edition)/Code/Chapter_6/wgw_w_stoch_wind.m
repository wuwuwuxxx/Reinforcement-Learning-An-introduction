function [Q,ets] = wgw_w_stoch_wind(alpha,sideII,sideJJ,s_start,s_end,wind,MAX_N_EPISODES)
% WGW_W_STOCH_WIND - Performs on-policy sarsa iterative action value funtion estimation 
% for the windy grid world example with kings moves (diagonal moves) such that the wind has 
% a stochastic component.
% 
% Note: there is no "wind" action (the action to just let the wind carry you somewhere)
% 
% Written by:
% -- 
...
%   an extra negative y component of the wind
%   no extra component
%   an extra positive y component of the wind
%
prob_und = ones(1,3)/3; 

% see what the wind does ... get index and then value: 
swi = sample_discrete( prob_und, 1, 1 ); ew = swi-2; % convert from index to value 

% convert state to row/column notation: 
ii = st(1); jj = st(2); 
