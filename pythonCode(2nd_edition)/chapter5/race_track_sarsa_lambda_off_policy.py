...
def get_rho(true_action, action, eps):
    if true_action == action:
        return 1/(1-eps+eps/actions)
    return 0
...
for k in xrange(500000):
    ...
    while True:
        ...
        rho = get_rho(action, policy[row, col, vv, vh], eps)
        # print rho
        E[row, col, vv, vh, action] = rho*(1-alpha)*E[row, col, vv, vh, action]+1
        Q += alpha*delta*E
        E *= alpha*lamb
        policy = np.argmax(Q, axis=4)
